  ...
  <div class="page__border">
          ...
          :style="[
            this.image
              ? {
                  backgroundImage:
                    'url(' +
                    require('@/assets/img/' + this.image + '.jpg') +
                    ')',
                }
    ...
    <transition name="fade2">
      <div v-if="this.showImage === true" class="quiz__section">
        ...
        <div class="quiz__sub-section">
          <button-quiz-answer
            :btnName="this.answerChoiceC"
            :disabledBtns="this.disabledBtns"
            @check-answer="this.checkAnswer"
          ></button-quiz-answer>
          <button-quiz-answer
            :btnName="this.answerChoiceD"
            :disabledBtns="this.disabledBtns"
            @check-answer="this.checkAnswer"
          ></button-quiz-answer>
        </div>
        <p class="quiz__info-txt">
          Question # {{ this.questionsCurrent }} of {{ this.questionsTotal }}
        </p>
      </div>
    </transition>
    <div v-if="this.showModal" class="quiz__modal">
      <div v-if="this.correct" class="quiz__modal-txt">
...
export default {
  ...
  emits: ["quizFinished", "setQuizScore"],
  components: {
    ButtonQuizAnswer,
  },
  data() {
  ...
  methods: {
    shuffleColorQuizData() {
      ...
      for (let i = 0; i < colorQuizData.length; i++) {
        const pickedNumber = Math.floor(
          Math.random() * colorQuizDataBuffer.length
        );
      ...
      setTimeout(() => {
        if (this.questionsCurrent < this.questionsTotal) {
          ...
          setTimeout(() => {
            this.$emit(
              "setQuizScore",
              Math.floor((this.questionsCorrect / this.questionsTotal) * 100)
            );
